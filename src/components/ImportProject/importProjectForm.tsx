...
import {importProject, getDirFileList} from 'src/api/project'
...
const creatDom = ({finishAction, cRef}: {finishAction?: (re: any) => any; cRef?: any}) => {
  ...
  const onFormFinish = async (values: any) => {
    ...
    const response = await importProject(values)
    const {code, msg, data} = response
    code === 0 ? message.success({content: msg, key}) : message.error({content: msg, key})
